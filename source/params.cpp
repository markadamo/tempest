...
extern void Tempest::parse_params() {
    ...
    //list and track required parameters
    /*
    std::map<char*, bool> paramParsed;
    std::vector<char*> reqParams;
    reqParams.push_back("digest_definition");
    reqParams.push_back("precursor_tolerance");
    reqParams.push_back("bin_width");
    */
    ...
    while (fgets(sLine, sizeof(sLine), fp)) {
        //replace > with null terminator to stop parsing line at comment symbol
        ...
        else if (strcmp(sParam, "fragment_bin_width") == 0) {
            if (sscanf(sLine, "fragment_bin_width %f \n", &Tempest::params.fFragmentTolerance) != 1) {
                fprintf(stderr, "\nERROR: Could not parse MS/MS fragment bin width\n\t>%s", sLine);
                Tempest::tempest_exit(EXIT_FAILURE);
            }

            if (Tempest::params.fFragmentTolerance <= 0) {
                fprintf(stderr, "\nERROR: Invalid MS/MS fragment bin width: %f\n\t>%s", Tempest::params.fFragmentTolerance, sLine);
                Tempest::tempest_exit(EXIT_FAILURE); 
            }
        }

        //changed this parameter to fragment_bin_width because ppm tolerance is unsupported.
        /*
